...
d   = 0.5;    et  = 2;
cor = 1;      tau = 0.5; % quantile

[X,Y,Xv,Yv,B_true] = Generatedata(n,p,q,s,d,et,cor); 
nv = size(Xv,1);      [p,q] = size(B_true);
B0 = zeros(p,q); 

smooth_type = input('Please choose the smoothing type {"lower" or "upper"}:\n','s')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
quan_para.kappa = 10^(-3);     quan_para.tau     = tau;
quan_para.xi    = 10^(-3);     quan_para.maxiter = 5000; 
quan_para.tol   = 10^(-5);     quan_para.line    = 0.7;
linesearch      = 1;
...
for i=1:l_lambda
quan_para.B0 = B0;
[B_new,Supp_B_new,Time] = Smooth_QuantileMML(X,Y,lambda(i),linesearch,quan_para,smooth_type);
...
MMlower_Smooth_Results = [MSE,  FP,  FN, CPU]
end
if strcmp(smooth_type,'upper')
MMupper_Smooth_Results = [MSE,  FP,  FN, CPU]
end
